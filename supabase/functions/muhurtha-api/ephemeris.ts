...
export function nakshatraPadaFromSiderealLon(lonDeg: number): {
  ...
  return { idx, name: NAKSHATRAS[idx] as string, pada };
}
...
export function siderealMetaFromNameOrMoon(
...
): { idx: number; name: string; pada: number } | null {
  ...
  return nakshatraPadaFromSiderealLon(moonLonDeg);
}